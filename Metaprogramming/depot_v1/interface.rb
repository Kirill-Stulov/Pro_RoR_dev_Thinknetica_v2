...
module StationsActions
  def show_stations
    ...
    #  это работает благодаря инстанс методу stations_collect из модуля InstanceCounter
...
module MainTrainsActions
  ...
  #  с помощью метода send вызываем на объекте train выбранный метод next_station или prev_station
  #  if а не case, т.к варианта всего 2 (1 или 2)
  def move_train_case(move, train)
    if (1..2).include?(move)
      train.send(move == 1 ? :next_station : :prev_station)
  ...
  def show_train_details
    loop do
      ...
      puts "--> Current station: #{selected_train7.current_station} \n--> Type: #{selected_train7.options[:type]}"
  ...
  def train_details(train)
    train.train_detail do |wagon| # передаем (не именованный) блок напрямую в метод train_detail
...
module RouteActions
  ...
  def show_all_routes
    Route.all_routes.each do |num, route|
      print "#{num}: "
  ...
  def create_route_menu
    puts "Choose and type first station name from list above \n #{show_stations}"
    st1 = gets.chomp
    puts "Choose last station name from list above \n #{show_stations}"
    st2 = gets.chomp
  ...
  def create_route(st1, st2)
    ...
    puts "Total routes created: #{Route.all_instances} \n"
    show_all_routes
  end
  ...
  def select_r(train)
    ...
    selected_route = Route.all_routes.fetch(select_route)
  ...
  def add_or_del_from_route
    ...
    chosen_route = Route.all_routes.fetch(route_select)
  ...
  def add_st(route)
    puts "Choose and type station from list above: #{show_stations}"
    station_to_add_input = gets.chomp

    # валидация что добавляемая станция существует
    self.class.validate_st_name(station_to_add_input) # self.class так обращаемся к методу класса из инстанс метода

    # почему-то return в самом validate_st_name не срабатывает на методе add_st(route), пришлось добавить эту строку,
    #  у del_st(route) проблем нет, пототому что проблем с удалением nil нет и после валидации все срабатывает
    return unless Station.existing_station?(station_to_add_input)

    station_to_add = Station.all_stations.find { |st| st.name == station_to_add_input }
    route.add_station(station_to_add.name)
    puts "Updated route: #{route.stations}"
  end

  # 'add_or_del_from_route' method helper3
  def del_st(route)
    puts "Choose and type station to delete: #{route.stations}"
    station_to_del_input = gets.chomp

    # валидация что удаляемая станция существует
    self.class.validate_st_name(station_to_del_input)
...
class Interface
  ...
  def initialize
    ...
    # тестовые данные, раскомментировано чтобы показать работу валидаций
    Station.new('tas')
    Station.new('chi')
    Station.new('yan')
    Station.new('pit')
    ...
    PassengerTrain.new('new-34')
    PassengerTrain.new('new-37')
    CargoTrain.new('new-38')

    Route.new(Station.all_stations[0].name, Station.all_stations[1].name)
    Train.all_trains[1].accept_route(Route.all_routes[1]) # поезд "abc-12" принимает маршрут №1
    Train.all_trains[2].accept_route(Route.all_routes[1]) # поезд "567-78" принимает маршрут №1
    ...
    # p Train.trains[1].instance_variable_get "@#{name}_history".to_sym
  end
  ...
  def answers
    ...
    { '1' => method(:create_station), '2' => method(:create_trains),
      ...
      '15' => method(:traind), '16' => method(:change_instances_history) }
  end

  # выводит экземпляры всех классов
  def show_all_instances
    puts "Station instances: #{Station.all_instances}"
    puts "Route instances: #{Route.all_instances}"
    puts "Train instances: #{Train.all_instances}"
    puts "Cargo train instances: #{CargoTrain.all_instances}"
    puts "Passenger train instances: #{PassengerTrain.all_instances}"
    puts "Train v-s: #{Train.instance_variables}"
    # puts "Stations v-s: #{Station.instance_variable_get :@instances}"
  end

  alias showin show_all_instances

  # метод 1 для проверки метода attr_accessor_with_history
  #  используется в меню №16
  #  выводит текущую инстанс перменную
  #  выводит предыдущие исторические значения этой инстанс переменной
  def change_instances_history
    puts "Current num:#{Train.all_trains[0].number}"
    Train.all_trains[0].number = '888-88'
    puts "Current num:#{Train.all_trains[0].number}"
    # p Train.all_trains[0].number_history
    # puts '-----Station------'
    puts Station.all_stations[0].name
    Station.all_stations[0].name = 'Pas'
    puts Station.all_stations[0].name
    puts "Station history: #{Station.all_stations[0].name_history}"
    # puts ##### Strong Accessor by speed check #############
    # Train.all_trains[0].speed = 23
    # puts "Current speed:#{Train.all_trains[0].speed}"
    # Train.all_trains[2].options[:wagons][0].manufacturer = 'S&P'
    # puts Train.all_trains[2].options[:wagons][0].manufacturer
  end

  # метод 2 для проверки метода attr_accessor_with_history
  #  выводит текущую инстанс перменную
  #   выводит предыдущие исторические значения этой инстанс переменной
  # def show_instances_history(name)
  #   puts "Train current instances value"
  #   puts "Trains historical instances values"
  #   puts "Station current instances value"
  #   puts "Station historical instances values"
  # end
end
