...
class Station
  ...
  def show_trains_by_type(type)                                # Может возвращать список поездов на станции по типу: кол-во грузовых, пассажирских. тип передаем в качестве аргумента в метод
    @trains.count { |train| train.type == type }            # возвращает количество поездов по типу
...
class Train
  ...
  def show_next_station                                     # может возвращать следующую станцию, на основе маршрута
    next_st_index = @tr_route.stations.index(@current_station) + 1   # вычисляем индекс следующей станции, для этого находим индекс текущей станции в массиве станций @tr_route и добавляем 1 
    next_station = @tr_route.stations[next_st_index]              # вычисляем по индексу следующую станцию в маршруте
    return next_sation
  end

  def show_prev_station 
    prev_st_index = @tr_route.stations.index(@current_station) - 1
    prev_station = @tr_route.stations[prev_st_index]
    return prev_station
  end
