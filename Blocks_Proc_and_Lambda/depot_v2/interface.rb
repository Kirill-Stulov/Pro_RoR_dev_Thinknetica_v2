...
class Interface
  ...
  # Метод для вывода детальной информации по всем поездам на станции
  def show_station_details
    puts "Show all trains on station menu"                                      # начало меню для выбора станции
    puts "..."
    puts "Type station name from the list below"
    @stations.map { |obj| puts obj.name }
    puts "..."
    selected_station_input = gets.chomp
    selected_station = @stations.find { |st| st.name == selected_station_input }  # конец меню для выбора станции

    trains_block = lambda do |trains|                                                                                   # создаем переменную, которая содержит блок, например через lambda (метод короткой однострочной записи лямбды смотри в how_to_write_functions_which_accepts_block.rb). В блоке передаем переменную trains, которая содержит массив поездов, перебираем его в следующей строке с выводом необходимой информации
      trains.each { |train| puts "-> Train ##{train.number}; Type:#{train.type}; Vagons:#{train.wagons.length}" }        # выводим в формате: Номер поезда, тип, кол-во вагонов
    end

    selected_station.station_details(trains_block)                                                                      # вызываем метод station_details и передаем ему блок для вывода данных о поездах на станции
  end
  ...
  def show_train_details  
    vagons_block = lambda do |vags|                                         # создаем блок через lambda. тот-же вариант что и trains_block
      vags.each do |vag|
        case vag.type                                                       # блок универсальный и выдает информацию по вагону в зависимости от его типа
        when :passenger                          
    ...
    loop do                                  
      ...
      when '0'
        ...
        selected_train7.train_details(vagons_block)                             # вызываем метод train_details и передаем ему блок для вывода данных о вагонах в поезде
      end
    end
  end
