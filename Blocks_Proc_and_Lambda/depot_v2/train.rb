...
class Train
  ...
  def train_details(block)                          # метод принимает в качестве аргумента блок как объект. Блок (задан в методе интерфейса show_train_details) проходит по всем вагонам @wagons поезда, и выводит детализацию по вагонам
      block.call(self.wagons)                                 # self нужен, чтобы можно было обратиться к методу (массиву) wagons поезда, на котором вызывается метод train_details в интерфесе (меню 15)
  end

# def train_details(wagons, block)                          # метод, который принимает блок и проходит по всем вагонам поезда, передавая каждый объект вагона в блок.
#     block.call(self.wagons)                                 # self нужен, чтобы можно было обратиться к методу (массиву) wagons поезда, на котором вызывается метод train_details в интерфесе (меню 15)
# end
