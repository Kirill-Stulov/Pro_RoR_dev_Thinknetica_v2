...
module InstanceCounter
  ...
  module ClassMethods
    attr_accessor :instances, :stations, :routes #(new)              # создаем и подключаем переменную класса instances, которая будет хранить кол-во экземпляров класса
   
    def instances                                            # метод класса - возвращает кол-во экземпляров данного класса
      @instances 
    end

    def stations #(new)                   # метод класса - возвращает кол-во экземпляров данного класса (возвращает все созданные станции класса Station). Этот дополнительный метод класса понадобился для (валидации) в методах validate! классов Station и Route 
      @stations ||= []
    end 

    def routes                            # не используется
      @routes #||= []                        # тут нужно было присвоить пустому массиву заранее, иначе при первом вызове - не будет срабатывать метод Route.routes.any? из метода existing_route? в route.rb, т.к массива для проверки просто не будет существовать при самом первом создании объекта
    end
  ...
  module InstanceMethods   
    ...
    def stations_collect    #(new)          # метод подключается в конструкторе класса Station и кладет каждый созданный экземпляр в массив
      self.class.stations ||= [] #(new)
      self.class.stations << self #(new)
    end

    def routes_collect                        # этот метод не используется, использую переменную класса @@routes в самом классе route  файла route.rb, а зря
      self.class.routes ||= []
      self.class.routes << self
    end

    protected                 # доступ к методам инстанс методам должен быть ограничен, но при этом в наследовании метод должен работать, потому protected (на самом деле, private тожк будет работать)
