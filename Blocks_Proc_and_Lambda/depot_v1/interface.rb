...
class Interface
  ...
  def initialize
    ...
    @trains[1].accept_route(@routes[1]) # поезд принимает маршрут №1 
    @trains[2].accept_route(@routes[1])
  ...
  def call  
    loop do
      ...
      when 14
        all_stations_details                                               # выводит подробный список станций и поездов на этих станциях, внутри метод stations_details - который принимает блок и проходит по всем станциям и поездам на этих станциях, передавая каждый поезд в блок. 17:00 - 19:00.
      when 15 
        all_trains_details                                                 # выводит подробный список поездов их вагонов, внутри метод all_trains_details - который принимает блок и проходит по всем поездам и их вагонам, передавая каждый вагон в блок. 17:00 - 19:00
      end
    end
  end
    ...
    loop do
      ...
      when '0'
        ...
        when :cargo
          ...
          puts "- occupied volume: #{selected_wagon2.show_occupied_volume} tons out of #{selected_wagon2.initial_volume}" # показать занятый объем вагона в соотношении с общим объемом в тоннах
        when :passenger
          selected_wagon2.occupy_seat                                                         # тут, в отличии от грузового вагона у нас по умолчанию сразу занимается одно место
          puts "- occupied seats: #{selected_wagon2.show_occupied_seats} out of #{selected_wagon2.initial_seats}" # показать количество занятых мест в соотношении с общим кол-вом мест 
        end
      end
    end
  end
