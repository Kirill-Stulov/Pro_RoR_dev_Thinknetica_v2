...
module StationsActions
  def show_stations
    ...
    puts Station.stations.map(&:name)
  ...
  def create_station
    loop do
      ...
      puts "Total stations created: #{Station.stations.size}"
...
# module contains all methods related to trains actions
module TrainsActions
  def find_train
    loop do
      ...
      puts "Search results: #{Train.trains.find { |tr| tr.number == selected_train }}"
    end
  end
  ...
  def create_trains
    loop do
      ...
      # If train_input equals 1, call 'create_cargo'
      #  if not, check if equals 2, if so, call 'create_passenger'
      #  if none of conditions met, assign 'Wrong input!' to result.
      result = train_input == '1' ? create_cargo : (train_input == '2' ? create_passenger : 'Wrong input!')
      puts result
    end
  end
  ...
  def create_cargo
    ...
    puts "Total trains created: #{Train.trains.size}"
    @show_trains.call
  end
  ...
  def move_train_back_forward
    loop do
      ...
      selected_train = Train.trains.find { |tr| tr.number == select_train }
  ...
  def show_train_details
    loop do
      ...
      selected_train7 = Train.trains.find { |tr| tr.number == select_train }
...
module WagonsActions
  ...
  def finding_train(train)
    selected_train = Train.trains.detect { |tr| tr.number.eql?(train) }
...
module RouteActions
  ...
  def adding_or_deleting(route)
    ...
    #  If none of conditions met, assign 'Wrong input' to result.
    result = add_or_del == '1' ? add_st(route) : (add_or_del == '2' ? del_st(route) : 'Wrong input')
    # If result not nil, output value of result. Otherwise output 'Wrong input'
    puts result
  end
  ...
  def add_st(route)
    ...
    station_to_add = Station.stations.find { |st| st.name == station_to_add_input }
...
class Interface
  ...
  def initialize
    ...
    @show_trains = -> { Train.trains.map { |obj| puts "Train # #{obj.number} #{obj.options[:type]}" } }
    # class instance variable to store data in lambda (pr proc)
    #  returns text in menu methods
    @select_tr_text = lambda do
      ...
      Train.trains.map { |obj| puts "Train # #{obj.number} #{obj.options[:type]}" }
      puts '...'
    end
    ...
    # Route.new(Station.stations[0].name, Station.stations[1].name)
    # Train.trains[1].accept_route(Route.routes[1]) # поезд "abc-12" принимает маршрут №1
    # Train.trains[2].accept_route(Route.routes[1]) # поезд "567-78" принимает маршрут №1
    # Train.trains[1].add_wagon(CargoWagon.new(@wagon_nums += 1)) # поезду "abc-12" добавляется вагон
    # Train.trains[2].add_wagon(PassengerWagon.new(@wagon_nums += 1)) # поезду "567-78" добавляется вагон
